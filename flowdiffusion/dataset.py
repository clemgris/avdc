...
class SequentialDatasetNp(Dataset):
    def __init__(
        ...
        self.sequences = obss
        self.tasks = tasks
        self.transform = T.Compose([T.Resize(target_size), T.ToTensor()])
...
class SequentialDataset(SequentialDatasetNp):
    def __init__(
        self,
        path="../datasets/frederik/berkeley",
        sample_per_seq=7,
        target_size=(128, 128),
    ):
        print("Preparing dataset...")
...
class SequentialDatasetVal(SequentialDataset):
    def __init__(
        self, path="../datasets/valid", sample_per_seq=7, target_size=(128, 128)
    ):
        print("Preparing dataset...")
        sequence_dirs = sorted(
            [d for d in os.listdir(path) if "json" not in d], key=lambda x: int(x)
        )
        self.sample_per_seq = sample_per_seq
        self.sequences = []
        self.tasks = []
...
class SSRDatasetNp(SequentialDatasetNp):
    def __init__(
        ...
        self.downsample_tfm = T.Compose(
            [T.Resize(in_size), T.Resize(target_size), T.ToTensor()]
        )
    ...
    def __getitem__(self, idx):
        ...
        x_cond = torch.cat(
            [self.downsample_tfm(Image.fromarray(s)) for s in samples][1:], dim=0
        )
...
class SSRDatasetVal(SequentialDatasetVal):
    ...
    def __getitem__(self, idx):
        ...
        x_cond = torch.cat(
            [self.downsample_tfm(Image.open(s)) for s in samples][1:], dim=0
        )
        ### apply noise on x_cond
        cond_noise = torch.randn_like(x_cond) * 0.2
        x_cond = x_cond + cond_noise
...
class MySeqDatasetMW(SequentialDataset):
    def __init__(
        self, path="../datasets/dataset_0513", sample_per_seq=8, target_size=(64, 64)
    ):
        print("Preparing dataset...")
        self.sample_per_seq = sample_per_seq
        ...
        self.transform = T.Compose(
            [T.CenterCrop((128, 128)), T.Resize(target_size), T.ToTensor()]
        )
        print("Done")
...
class SequentialDatasetv2(Dataset):
    def __init__(
        ...
        sequence_dirs = glob(f"{path}/**/metaworld_dataset/*/*/*/", recursive=True)
        print(f"Found data in {len(sequence_dirs)} directories")
        self.tasks = []
        self.sequences = []
        for seq_dir in sequence_dirs:
            ...
            seq = sorted(
                glob(f"{seq_dir}*.png"),
                key=lambda x: int(x.split("/")[-1].rstrip(".png")),
            )
            self.sequences.append(seq)
    ...
    def get_samples(self, idx):
        ...
        if self.frame_skip is None:
            ...
            samples = [
                ...
                for i in range(
                    start_idx,
                    start_idx + self.frame_skip * self.sample_per_seq,
                    self.frame_skip,
                )
            ]
        return [seq[i] for i in samples]
    ...
    def __getitem__(self, idx):
        try:
            ...
            print("x", x.shape, "xcond", x_cond.shape)
            task = self.tasks[idx]
            return x, x_cond, task
...
class SequentialNavDataset(Dataset):
    def __init__(
        self, path="../datasets/valid", sample_per_seq=8, target_size=(64, 64)
    ):
        print("Preparing dataset...")
        self.sample_per_seq = sample_per_seq
        ...
        self.transform = video_transforms.Compose(
            [video_transforms.Resize(target_size), volume_transforms.ClipToTensor()]
        )
        ...
        self.frameid2seq_subid = [
            f - self.frameid2seqid.index(self.frameid2seqid[f])
    ...
    def get_samples(self, idx):
        ...
        samples = [
            i if i < len(seq) else -1
            for i in range(start_idx, start_idx + self.sample_per_seq)
        ]
        return [seq[i] for i in samples]
...
class MySeqDatasetReal(SequentialDataset):
    def __init__(
        self,
        path="../datasets/dataset_0606/processed_data",
        sample_per_seq=7,
        target_size=(48, 64),
    ):
        print("Preparing dataset...")
...
class lerobotDataset(Dataset):
    def __init__(self, cfg: DictConfig):
        self.expert_demonstrations = self.load_expert_dataset(cfg)
        valide_index = list(
            range(
                0,
                len(self.expert_demonstrations),
                cfg.model.pred_horizon + cfg.model.obs_horizon,
            )
        )
        self.dataset = torch.utils.data.Subset(self.expert_demonstrations, valide_index)
        self.transform = T.Compose([T.Resize((128, 128)), T.ToTensor()])
        episodes = json.load(
            open(
                os.path.join(
                    cfg.expert_data.root, cfg.expert_data.dataset_id, "episodes.json"
                )
            )
        )
        self.goals = {v["episode_index"]: v["task"] for v in episodes}
    ...
    def load_expert_dataset(
        ...
        if timestamps:
            delta_timestamps = {
                ...
                "observation.next_image": eval(
                    cfg.delta_timestamps.observation_next_image
                ),
                "observation.next_state": eval(
                    cfg.delta_timestamps.observation_next_state
                ),
    ...
    def __getitem__(self, idx):
        ...
        x_cond = samples["observation.image"]
        x = samples["observation.next_image"]
        x_cond = x_cond.squeeze(0)
        x = rearrange(x, "f c h w -> (f c) h w")
        ep_idx = samples["episode_index"].item()
        task = self.goals[ep_idx]
        return x, x_cond, task
